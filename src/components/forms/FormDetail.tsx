...
export function FormDetail({ form: initialForm, onBack }: FormDetailProps) {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const response = await SubmissionsService.getFormSubmissions(form.id, { 
        page: currentPage, 
        limit 
      });
      setSubmissions(response.submissions);
      setTotalPages(response.pagination.totalPages);
      setTotalSubmissions(response.pagination.totalSubmissions);
    } catch (error) {
  ...
  const handleDeleteSubmission = (submission: FormSubmission) => {
    setSelectedSubmission(submission);
    setShowSubmissionDeleteModal(true);
  };
  ...
  const handleDeleteSuccess = () => {
    setShowSubmissionDeleteModal(false);
    setSelectedSubmission(null);
    fetchSubmissions();
  };
  ...
  return (
    <div className="space-y-6">
                        ...
                        <div>
                          <p className="text-sm text-gray-700">
                            ...
                            <span className="font-medium">
                              {Math.min(currentPage * limit, totalSubmissions)}
                            </span>{' '}
      ...
      {showSubmissionEditModal && selectedSubmission && (
        <SubmissionEditModal
      ...
      {showSubmissionDeleteModal && selectedSubmission && (

...
const validateUUID = [
  param("formId").isUUID().withMessage("Invalid ID format"),
];
...
const validateFormResponses = async (formId, responses) => {
  ...
  for (const field of fields) {
    ...
    if (
      ...
      const values = Array.isArray(responses[field.id])
        ? responses[field.id]
        : [responses[field.id]];
      if (field.options) {
        for (const value of values) {
          if (!field.options.includes(value)) {
            ...
            if (!field.allow_other) {
              errors.push(
                `${field.label} contains an invalid option: ${value}`,
              );
            }
          }
        }
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (form.max_submission_count > 0) {
        ...
        if (
          totalSubmissionsResult.rows[0].total_submissions >=
          form.max_submission_count
        ) {
          throw new AppError(
      ...
      if (
        ...
        error.code === "UNIQUE_FIELD_REQUIRED" ||
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      formResult.rows[0].created_by !== req.user.id
    ) {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.user.role !== "super_admin" && form.created_by !== userId) {
      throw new AppError(
        "You can only export submissions for your own forms",
        403,
        "FORM_ACCESS_DENIED",
      );
    }
    ...
    const filename = `${form.title.replaceAll(/[^a-zA-Z0-9]/g, "_")}_submissions_${new Date().toISOString().split("T")[0]}.xlsx`;
    res.setHeader(
      "Content-Type",
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    );
...
router.put(
  ...
  authenticateToken,
  [
    param("id").isUUID().withMessage("Invalid submission ID format"),
    body("responses").isObject().withMessage("Responses must be an object"),
  ],
  handleValidationErrors,
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new AppError(
        "You can only update submissions for your own forms",
        403,
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw new AppError(
        "You can only delete submissions for your own forms",
        403,
        "SUBMISSION_ACCESS_DENIED",
      );
    }
